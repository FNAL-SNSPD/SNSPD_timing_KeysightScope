'''
Code to take PCR and DCR with n number of channels
records count per second for each channel and coincidences between each channel
Christina Wang 11/27/2024
...
##########################################	
### define voltages and data taking time in the channels
##########################################
voltages = np.round([0.140,0.145,0.150,0.155,0.160,0.170,0.180,0.190,0.200,0.210,0.220, 0.23],3)
print("###################################################")
print("####### Voltages to scan #######")
print("###################################################")
print(voltages)
print(" ")


if os.getenv("USER") == "fqnet": cryo = "ADR"
elif os.getenv("USER") == "fcctt": cryo = "spot1"
else: exit("CAN NOT FIND USER")
print(" ")
print(f"cryostat used is {cryo}")
print(" ")
...
--vPos1 {vPos_ch1} --vPos2 {vPos_ch2} --vPos3 {vPos_ch3} --vPos4 {vPos_ch4}"
        	        print(cmd)
        	        os.system(cmd)
        	        turnOff(bias_ch)
